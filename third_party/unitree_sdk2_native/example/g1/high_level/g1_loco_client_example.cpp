...
int main(int argc, char const *argv[]) {
  ...
  for (const auto &arg_pair : args) {
    ...
    if (arg_pair.first == "set_velocity") {
      ...
      auto ret = client.SetVelocity(vx, vy, omega, duration);


      std::cout << "SetVelocity return code: " << ret << std::endl;
    ...
    if (arg_pair.first == "stop_move") {
      auto ret = client.StopMove();

      std::cout << "StopMove return code: " << ret << std::endl;
    }
