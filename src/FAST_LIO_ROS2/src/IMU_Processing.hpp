...
class ImuProcess
{
 public:
  ...
  void set_map_yaw_deg(double yaw_deg);
...
void ImuProcess::set_map_yaw_deg(double yaw_deg)
{
  map_yaw_deg_ = yaw_deg;
}
...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
  ...
  // [FAST-LIO] gravity alignment + fixed world yaw
  Eigen::Quaterniond gravity_q = Eigen::Quaterniond::Identity();
  V3D gravity_world = -mean_acc / mean_acc.norm() * G_m_s2;

  if (gravity_align_en_)
  {
    gravity_q = Eigen::Quaterniond::FromTwoVectors(
      mean_acc, Eigen::Vector3d::UnitZ());
    gravity_world = V3D(0.0, 0.0, -G_m_s2);
  }

  constexpr double DEG_TO_RAD = 0.017453292519943295769;
  const double yaw_rad = map_yaw_deg_ * DEG_TO_RAD;
  const Eigen::Quaterniond yaw_q(
    Eigen::AngleAxisd(yaw_rad, Eigen::Vector3d::UnitZ()));

  const Eigen::Quaterniond init_q = (yaw_q * gravity_q).normalized();
  init_state.rot = init_q;

  if (!gravity_align_en_)
  {
    gravity_world = yaw_q * gravity_world;
  }
